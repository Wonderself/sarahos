...
export default function StrategyPage() {
  ...
  return (
    <AuthRequired pageName="Strategie">
      ...
      <div style={{ display: 'flex', gap: 0, marginBottom: 24, borderBottom: '2px solid #E5E5E5', overflowX: 'auto' }}>
        {TABS.map(tab => (
          <button
            ...
            style={{
              padding: isMobile ? '10px 12px' : '10px 18px', fontSize: 13, fontWeight: activeTab === tab.id ? 700 : 500, minHeight: 44,
                ...
                {folders.filter(f => f.items.length > 0).map(folder => {
                  ...
                  return (
                    <div key={folder.id} className="flex items-center gap-8">
                      ...
                      <span className="text-sm font-semibold" style={{ width: isMobile ? 'auto' : 140, flexShrink: 0, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{folder.name}</span>
            ...
            {AGENT_STRATEGIES.map(strat => {
              ...
              return (
                <button
                  ...
                  style={{
                    padding: '8px 14px', minHeight: 44, fontWeight: isActive ? 700 : 500,
