          ...
          {PIPELINE.map(status => {
            ...
            return (
              <div key={status} style={{
                ...
                background: count > 0 ? STATUS_COLORS[status] + '15' : 'var(--bg-secondary)',
                border: `1px solid ${count > 0 ? STATUS_COLORS[status] + '40' : 'var(--border-primary)'}`,
                textAlign: 'center',
              }}>
                <div style={{ fontSize: 15, fontWeight: 700, color: STATUS_COLORS[status] }}>{count}</div>
                <div style={{ fontSize: 10, color: 'var(--text-tertiary)', marginTop: 2 }}>{STATUS_LABELS[status]}</div>
              </div>
            );
          })}
        </div>
              ...
              {campaigns.map(c => (
                <div
                  ...
                  style={{
                    ...
                    borderColor: selectedCampaign?.id === c.id ? 'var(--accent)' : undefined,
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                      {(c.posts_count ?? 0) > 0 && (
                        <span style={{ fontSize: 11, color: 'var(--text-tertiary)' }}>{c.posts_count} post{(c.posts_count ?? 0) > 1 ? 's' : ''}</span>
                      )}
                      {c.status === 'draft' && (
                        <button
                  ...
                  {c.description && (
                    <div style={{ fontSize: 12, color: 'var(--text-tertiary)', marginTop: 6, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {c.description}
                    </div>
                  )}
                </div>
        ...
        {selectedCampaign && (
          <div className="card" style={{ padding: 20, alignSelf: 'start', position: 'sticky', top: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
              ...
              <button onClick={() => setSelectedCampaign(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 18, color: 'var(--text-tertiary)', flexShrink: 0 }}>×</button>
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 14 }}>
              ...
              {selectedCampaign.objective && (
                <span style={{ fontSize: 11, color: 'var(--text-tertiary)' }}>· {OBJECTIVE_LABELS[selectedCampaign.objective]}</span>
              )}
            </div>

            {selectedCampaign.description && (
              <p style={{ fontSize: 12, color: 'var(--text-tertiary)', marginBottom: 14, lineHeight: 1.5 }}>{selectedCampaign.description}</p>
            )}
            ...
            <div style={{ borderTop: '1px solid var(--border-primary)', paddingTop: 14 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                <span style={{ fontSize: 13, fontWeight: 600 }}>Posts ({posts.length})</span>
              ...
              {postsLoading ? (
                <div className="text-tertiary animate-pulse" style={{ fontSize: 12, textAlign: 'center', padding: '12px 0' }}>Chargement...</div>
              ) : posts.length === 0 ? (
      ...
      {showModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
          ...
          <div className="card" style={{ width: '100%', maxWidth: 520, padding: 28 }}>
            <h3 style={{ fontSize: 17, fontWeight: 700, marginBottom: 20 }}>Nouvelle campagne</h3>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div>
                <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-secondary)', display: 'block', marginBottom: 6 }}>Plateformes *</label>
                  ...
                  {(Object.keys(PLATFORM_LABELS) as Platform[]).map(p => (
                    <button
                      ...
                      style={{
                        ...
                        border: form.platforms.includes(p) ? '1.5px solid var(--accent)' : '1.5px solid var(--border-primary)',
                        background: form.platforms.includes(p) ? 'var(--accent)' : 'var(--bg-secondary)',
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-secondary)', display: 'block', marginBottom: 4 }}>Objectif</label>
                ...
                <div>
                  <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-secondary)', display: 'block', marginBottom: 4 }}>Budget (€)</label>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-secondary)', display: 'block', marginBottom: 4 }}>Date début</label>
                  <input className="input" type="date" value={form.start_date} onChange={e => setForm(p => ({ ...p, start_date: e.target.value }))} style={{ width: '100%' }} />
                </div>
                <div>
                  <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-secondary)', display: 'block', marginBottom: 4 }}>Date fin</label>
      ...
      {showPostModal && selectedCampaign && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
          ...
          <div className="card" style={{ width: '100%', maxWidth: 460, padding: 28 }}>
            <h3 style={{ fontSize: 17, fontWeight: 700, marginBottom: 20 }}>Ajouter un post</h3>

            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div>
                <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-secondary)', display: 'block', marginBottom: 4 }}>Plateforme</label>
              ...
              <div>
                <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-secondary)', display: 'block', marginBottom: 4 }}>Contenu *</label>
              ...
              <div>
                <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-secondary)', display: 'block', marginBottom: 4 }}>Date de publication</label>
