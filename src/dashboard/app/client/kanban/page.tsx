...
export default function KanbanPage() {
  ...
  return (
    <div style={pageContainer(isMobile)}>
      {/* Header */}
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20, flexWrap: 'wrap', gap: isMobile ? 8 : 12 }}>
        <div>
          <div style={headerRow()}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <span style={{ ...CU.badge }}>{totalTasks} tâches</span>
          {tasksByCol.map(c => (
            <span key={c.name} style={{ fontSize: 10, color: CU.textMuted }}>{c.name}: {c.count}</span>
          ))}
        </div>
      </div>
      ...
      <div style={toolbar()}>
        <select value={data.activeBoardId} onChange={e => persist({ ...data, activeBoardId: e.target.value })}
          style={{ ...CU.select, height: 32, fontSize: 12 }}>
          {data.boards.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}
        </select>
        <button onClick={() => setShowBoardForm(true)} style={{ ...CU.btnSmall }}>➕ Tableau</button>
        {data.boards.length > 1 && board && (
          <button onClick={() => deleteBoard(board.id)} style={{ ...CU.btnSmall, color: CU.danger }}>🗑️ Suppr. tableau</button>
        )}
        <button onClick={() => setShowColForm(true)} style={{ ...CU.btnSmall }}>➕ Colonne</button>
        <div style={{ flex: 1 }} />
        {/* Filters */}
        <select value={filterPriority} onChange={e => setFilterPriority(e.target.value)} style={{ ...CU.select, height: 32, fontSize: 11 }}>
          <option value="all">Priorité: Toutes</option>
          {Object.entries(PRIORITY_MAP).map(([k, v]) => <option key={k} value={k}>{v.emoji} {v.label}</option>)}
        </select>
        <select value={filterLabel} onChange={e => setFilterLabel(e.target.value)} style={{ ...CU.select, height: 32, fontSize: 11 }}>
      ...
      {showBoardForm && (
        <div style={{ ...CU.card, padding: 16, marginBottom: 16, display: 'flex', gap: 8, alignItems: 'center' }}>
          <input value={newBoardName} onChange={e => setNewBoardName(e.target.value)} placeholder="Nom du tableau" style={{ ...CU.input, maxWidth: 250 }} />
          <button onClick={createBoard} style={CU.btnPrimary}>Créer</button>
          <button onClick={() => setShowBoardForm(false)} style={CU.btnGhost}>Annuler</button>
        </div>
      )}
      ...
      {showColForm && (
        <div style={{ ...CU.card, padding: 16, marginBottom: 16, display: 'flex', gap: 8, alignItems: 'center' }}>
          <input value={newColName} onChange={e => setNewColName(e.target.value)} placeholder="Nom de la colonne" style={{ ...CU.input, maxWidth: 250 }} />
          <button onClick={addColumn} style={CU.btnPrimary}>Ajouter</button>
          <button onClick={() => setShowColForm(false)} style={CU.btnGhost}>Annuler</button>
        </div>
      )}

      {/* Task form modal */}
      {showTaskForm && (
        <div style={CU.overlay} onClick={() => setShowTaskForm(false)}>
          <div style={{ ...CU.modal, maxWidth: 500 }} onClick={e => e.stopPropagation()}>
            <h2 style={{ ...CU.sectionTitle, fontSize: 16, marginBottom: 16 }}>{editTaskId ? 'Modifier' : 'Nouvelle'} tâche</h2>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div><label style={CU.label}>Titre</label><input value={taskTitle} onChange={e => setTaskTitle(e.target.value)} style={CU.input} /></div>
              <div><label style={CU.label}>Description</label><textarea value={taskDesc} onChange={e => setTaskDesc(e.target.value)} rows={2} style={CU.textarea} /></div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div><label style={CU.label}>Priorité</label>
                  <select value={taskPriority} onChange={e => setTaskPriority(e.target.value as Priority)} style={CU.select}>
                    {Object.entries(PRIORITY_MAP).map(([k, v]) => <option key={k} value={k}>{v.emoji} {v.label}</option>)}
                  </select>
                </div>
                <div><label style={CU.label}>Échéance</label><input type="date" value={taskDue} onChange={e => setTaskDue(e.target.value)} style={CU.input} /></div>
              </div>
              <div><label style={CU.label}>Colonne</label>
                <select value={taskColumnId} onChange={e => setTaskColumnId(e.target.value)} style={CU.select}>
                  {columns.map(c => <option key={c.id} value={c.id}>{c.title}</option>)}
                </select>
              </div>
              <div>
                <label style={CU.label}>Assigné</label>
                <div style={{ display: 'flex', gap: 6, marginTop: 4 }}>
                  {ASSIGNEE_EMOJIS.map(e => (
                    <button key={e} onClick={() => setTaskAssignee(e)} style={{ width: 32, height: 32, borderRadius: 8, border: taskAssignee === e ? `2px solid ${CU.accent}` : `1px solid ${CU.border}`, background: CU.bg, fontSize: 16, cursor: 'pointer' }}>{e}</button>
                  ))}
                </div>
              </div>
              <div>
                <label style={CU.label}>Labels</label>
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 4 }}>
                  {LABELS.map(l => (
                    <button key={l.name} onClick={() => toggleLabel(l.name)}
                      style={{ padding: '4px 10px', borderRadius: 12, fontSize: 11, fontWeight: 600, cursor: 'pointer', border: taskLabels.includes(l.name) ? `2px solid ${l.color}` : `1px solid ${CU.border}`, background: taskLabels.includes(l.name) ? l.color + '20' : CU.bg, color: l.color }}>
                      {l.name}
                    </button>
                  ))}
                </div>
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 16 }}>
              <button onClick={() => setShowTaskForm(false)} style={CU.btnGhost}>Annuler</button>
      ...
      {renameColId && (
        <div style={{ ...CU.card, padding: 12, marginBottom: 12, display: 'flex', gap: 8, alignItems: 'center' }}>
          <input value={renameColVal} onChange={e => setRenameColVal(e.target.value)} style={{ ...CU.input, maxWidth: 200 }} />
          <button onClick={renameColumn} style={CU.btnPrimary}>OK</button>
          <button onClick={() => setRenameColId(null)} style={CU.btnGhost}>Annuler</button>
        </div>
      )}
      ...
      {!board ? (
        <div style={{ ...CU.card, ...CU.emptyState }}>
          ...
          <p style={CU.emptyTitle}>Aucun tableau</p>
          <p style={CU.emptyDesc}>Créez-en un ci-dessus.</p>
        </div>
      ) : (
          ...
          {columns.map(col => {
            ...
            return (
              <div key={col.id} style={{ minWidth: isMobile ? 260 : undefined }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, padding: '6px 8px', background: CU.bgSecondary, borderRadius: 8 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span style={{ fontSize: 13, fontWeight: 600, color: CU.text }}>{col.title}</span>
                    <span style={{ fontSize: 11, color: CU.textMuted }}>({colTasks.length})</span>
                  </div>
                  <div style={{ display: 'flex', gap: 2 }}>
                    ...
                    <button onClick={() => deleteColumn(col.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 12, color: CU.danger }} title="Supprimer">🗑️</button>
                  </div>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {colTasks.map(task => (
                    <div key={task.id} style={{ ...CU.card }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 6 }}>
                        <div style={{ fontWeight: 600, fontSize: 13, flex: 1, color: CU.text }}>{task.title}</div>
                        <span style={{ fontSize: 16 }}>{task.assignee}</span>
                      </div>
                      {task.description && <div style={{ fontSize: 11, color: CU.textSecondary, marginBottom: 6 }}>{task.description}</div>}
                      <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginBottom: 6 }}>
                        ...
                        {task.dueDate && <span style={{ fontSize: 10, color: CU.textMuted }}>📅 {task.dueDate}</span>}
                      </div>
                      {task.labels.length > 0 && (
                        <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginBottom: 6 }}>
                          {task.labels.map(l => {
                            const labelDef = LABELS.find(lb => lb.name === l);
                            return <span key={l} style={{ fontSize: 9, padding: '2px 6px', borderRadius: 8, background: (labelDef?.color ?? CU.textSecondary) + '18', color: labelDef?.color ?? CU.textSecondary, fontWeight: 600 }}>{l}</span>;
                          })}
                        </div>
                      )}
                      <div style={{ display: 'flex', gap: 4 }}>
                        <select value={task.columnId} onChange={e => moveTask(task.id, e.target.value)} style={{ ...CU.select, height: 22, fontSize: 9, padding: '0 2px', flex: 1 }}>
                          {columns.map(c => <option key={c.id} value={c.id}>{c.title}</option>)}
                        </select>
                        <button onClick={() => openEditTask(task)} style={{ ...CU.btnSmall, height: 22, fontSize: 9, padding: '0 4px' }}>✏️</button>
                        <button onClick={() => deleteTask(task.id)} style={{ ...CU.btnSmall, height: 22, fontSize: 9, padding: '0 4px', color: CU.danger }}>🗑️</button>
                      </div>
                    </div>
                  ))}
                  {colTasks.length === 0 && (
                    <div style={{ ...CU.card, textAlign: 'center', color: CU.textMuted, fontSize: 11 }}>Vide</div>
                  )}
                </div>
              </div>
            );
